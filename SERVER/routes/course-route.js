...
router.get("/",(req,res) =>{
    Course.find({}).populate("instructor",["username","email"]).then((course)=>{
        res.send(course);
    }).catch(()=>{
...
router.get("/instructor/:_instructor_id",(req,res) =>{
    ...
    Course.find({instructor: _instructor_id}).populate("instructor",[
        ...
        res.status(500).send("Can't get the course data")
    });
});

router.get("/findByName/:name",(req,res) =>{
    let { name } = req.params;
    Course.find({title: name}).populate("instructor",[
        "username",
        "email"
    ]).then((course) =>{
...
router.get("/student/:_student_id",(req,res) =>{
    ...
    Course.find({student: _student_id}).populate("instructor",[
        "username",
        "email"
    ]).then((courses) =>{
...
router.get("/:_id",(req,res) =>{
    let {_id} = req.params;
    Course.findOne({_id}).populate("instructor",["email"]).then((course) =>{
        res.send(course);
    }).catch(e =>{
        res.send(e);
    })
...
router.post("/", async (req,res)=>{
    ...
    try{
        ...
        res.status(400).send("Course can't be saved");
    }
});
...
router.delete("/:_id", async (req,res)=>{
    ...
    if(course.instructor.equals(req.user._id) || req.user.isAdmin()){
        ...
        return res.json({
            success: false,
            message: "Only the instructor of this course or web admin can delete this course"
        })
    }
})
